  ...
  */
  ...
  function migrate(s) {
    ...
    if (Array.isArray(s.banks)) {
      // 移除旧版内联的 10 道 pilot_ 题目（已被全量 z600 取代，避免重复）
      s.banks = s.banks.filter(b => !b || String(b.id).indexOf('pilot_') !== 0);
      s.banks.forEach(b => {
        if (!b) return;
    ...
    if (Array.isArray(s.wrong)) {
      s.wrong = s.wrong.filter(w => !w || String(w.id).indexOf('pilot_') !== 0);
      s.wrong.forEach(w => {
        if (!w) return;
